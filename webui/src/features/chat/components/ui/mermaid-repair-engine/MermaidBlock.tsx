...
export const MermaidBlock: React.FC<Props> = ({ codeString }) => {
  ...
  return (
    <>
      <div className="relative overflow-hidden rounded-2xl border border-slate-700 bg-[#1E1E1E] shadow-lg shadow-black/30">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-700/50 bg-slate-900 px-4 py-2">
          {/* Segmented Control */}
          <div className="relative flex rounded-xl bg-slate-800 p-0.5">
            <div
          ...
          <div className="flex gap-2">
            {viewMode === 'preview' && (
              <button
                onClick={openModal}
                className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-800 transition-all duration-200 hover:bg-slate-700"
                title="Full Screen"
              >
            ...
            {viewMode === 'code' && (
              <button
                onClick={handleCopy}
                className="relative flex h-8 w-8 items-center justify-center rounded-lg bg-slate-800 transition-all duration-200 hover:bg-slate-700"
              >
                <div
        ...
        <div className="relative overflow-hidden">
          {/* Preview */}
          <div
            className={`transition-all duration-300 ease-in-out ${viewMode === 'preview' ? 'translate-y-0 opacity-100' : 'pointer-events-none absolute inset-0 -translate-y-2 opacity-0'}`}
          >
            <div
              className="cursor-zoom-in bg-slate-900 p-1"
              onClick={openModal}
            >
          ...
          {/* Code */}
          <div
            className={`transition-all duration-300 ease-in-out ${viewMode === 'code' ? 'translate-y-0 opacity-100' : 'pointer-events-none absolute inset-0 translate-y-2 opacity-0'}`}
          >
            <SyntaxHighlighter
              style={vscDarkPlus}
              language="mermaid"
      ...
      {isModalOpen && (
        ...
        >
          <button
            onClick={() => setIsModalOpen(false)}
            className="absolute top-6 right-6 z-50 rounded-full bg-slate-800 p-2 text-slate-300 transition-colors hover:bg-slate-700 hover:text-white"
          >
            <X size={24} />
          </button>

          <div
            className={`flex h-full w-full items-center justify-center overflow-hidden ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
          ...
          >
            <div
              ...
              className="rounded-xl bg-slate-900 p-8 shadow-2xl"
            >
              <MermaidDiagram chart={codeString} />
            </div>
          </div>
